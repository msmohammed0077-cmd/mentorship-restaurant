...
**Identity sequences are out of sync.** `V2` and `V3` insert explicit primary keys into `GENERATED BY DEFAULT AS IDENTITY` columns, which does not advance the sequences. Until a migration runs `setval(...)` on the seeded tables, the first row the application inserts collides with a seeded id:
