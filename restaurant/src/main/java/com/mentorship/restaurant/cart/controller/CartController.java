...
public class CartController {
    ...
    public ResponseEntity<CartResponse> modifyItem(
            @PathVariable Long cartId, @PathVariable Long cartItemId,
            @Valid @RequestBody UpdateCartItemRequest request
    ) {
        return ResponseEntity.ok(cartService.modifyItem(cartId, cartItemId, request.getQuantity(), request.getNote()));
    }
}
